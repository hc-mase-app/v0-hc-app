...
export default function PJODashboard() {
  ...
  const loadData = () => {
    ...
    const pending = Database.getPendingRequestsForPJOBySite(user.site)
    setPendingRequests(pending.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()))

    const all = Database.getLeaveRequests().filter((r) => r.site === user.site)
    ...
    const siteStats = {
      total: all.length,
      pending: all.filter(
        (r) => r.status === "pending_atasan" || r.status === "pending_pjo" || r.status === "pending_hr_ho",
      ).length,
  ...
  useEffect(() => {
      ...
      if (activeTab === "pending") {
        ...
        setFilteredRequests(
          allRequests.filter((r) => {
            ...
            return history.some((h) => h.approverUserId === user?.id && h.approverRole === "pjo_site")
          }),
        )
      }
      return
    ...
    const filtered = baseRequests.filter((request) => {
      return (
        ...
        (request.jenisIzin?.toLowerCase() || "").includes(query) ||
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Level 2 approval</p>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Tinjau dan kelola semua pengajuan izin dari karyawan di site Anda</CardDescription>
